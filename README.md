...
### 1. 오버 엔지니어링보다 출시 시점을 우선한 정적 구조

인터넷 방송 밈은 유행 주기가 짧기 때문에
초기 단계에서는 확장성 높은 백엔드를 구성하는 것보다
사용자가 관심을 가질 때 서비스를 공개하는 것이 더 중요하다고 판단했습니다.

따라서 Supabase나 Firebase 같은 외부 DB를 도입하지 않고
기록을 `data.js`에서 관리하는 정적 구조로 시작해 Netlify에 바로 배포했습니다.

그 결과 유행이 확산되는 시점에 서비스를 빠르게 공개할 수 있었고
Netlify 정적 호스팅을 통해 별도의 애플리케이션 서버 없이
7일간 68,107 페이지뷰와 226GB의 트래픽을 서비스했습니다.

초기에는 커뮤니티 제보와 직접 검색을 통해 기록을 확인해 수동으로 등재했고
이후 네이버 폼을 통해 제보를 받아 검증 자료를 확인한 뒤 반영하는 방식으로 운영했습니다.

이후 리팩토링 과정에서는 이 운영 경험을 바탕으로
제출·검증·관리자 검토 과정을 OCI 서버로 분리해 반자동화했습니다.
공개 리더보드는 기존 정적 구조를 유지해
검토 서버와 사용자 트래픽을 서로 분리했습니다.
